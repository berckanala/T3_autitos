...
def furness(t, O, D, tol=1e-6, maxit=1000):  
    ...
    iters = 0
    while iters < maxit:
        row_sums = t.sum(axis=1, keepdims=True)  # Asegurar que row_sums tiene la misma dimensión que O
        col_sums = t.sum(axis=0, keepdims=True)  # Asegurar que col_sums tiene la misma dimensión que D
        ...
        bj = D / col_sums
        t = t * bj
        row_sums_after = t.sum(axis=1, keepdims=True)
        col_sums_after = t.sum(axis=0, keepdims=True)
        
        if np.max(np.abs(row_sums_after - O)) < tol and \
...
k_values = np.arange(0.001, 1.001, 0.001)

# Inicializar variables para almacenar el mejor k y el MSE mínimo
best_k = None
min_mse = float('inf')
...
for k in k_values:
    ...
    for i in range(10):  # Asumiendo que hay 10 zonas (ajusta si es necesario)
        ...
        for j in range(10):
            ...
            Tij = O2024[i] * D2024[j] * (cij ** -k) * np.exp(-beta * cij)  # Cálculo de Tij
            row.append(Tij)  # Añadir el resultado a la fila
        Tij_list.append(row)  # Añadir la fila a la lista principal

    # Normalizar Tij_list
    denominador = sum(sum(Tij_list[i][j] for j in range(10)) for i in range(10))
    numerador = O2024.sum()
    ...
    Tij = Tij_df.iloc[:-1, :-1].astype(float).values
    resultado = furness(Tij, O2024, D2024)
    resultado_df = pd.DataFrame(resultado, index=Tij_df.index[:-1], columns=Tij_df.columns[:-1])

    # Calcular el MSE
    mse = np.mean((resultado_df - df1_1) ** 2)

    # Actualizar el mejor k si el MSE es más bajo
    if mse < min_mse:
        min_mse = mse
        best_k = round(k, 3)  # Redondear k a 3 cifras decimales

# Mostrar el mejor valor de k y su MSE
print(f"El mejor valor de k es {best_k} con un MSE de {min_mse:.4f}")
